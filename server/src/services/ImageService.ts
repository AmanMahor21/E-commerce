...
export class ImageService {
  ...
  public async listFolders(limit: number = 0, marker: string = '', folderName: string = ''): Promise<any> {
    ...
    const files = fileUnsanitized.filter((file) =>
      notAllowFolder.length ? !notAllowFolder.includes(file) : true
    );
  ...
  public imageUpload(folderName: string = '', base64Image: any): Promise<any> {
    ...
    const allowedFileSizeInKb = Number(process.env.IMAGE_UPLOAD_SIZE) * 1024;

    if (sizeInKb > allowedFileSizeInKb) {
      throw new BadRequestError(
        `File size too large, must be lees than ${+Number(process.env.IMAGE_UPLOAD_SIZE)} mb`
      );
    }
    const filteredPath = path.normalize(folderName).replace(/^(\.\.(\/|\\|$))+/, '');
    console.log('filtered path -\n\n\n\n\n', filteredPath);
    const uploadFileDir = path.join(process.cwd() + '/' + 'uploads');
    const filePath = path.join(uploadFileDir + '/' + filteredPath);
    ...
    return new Promise((resolve, reject) => {
      fs.writeFile(filePath, base64Image, 'base64', (err) => {
        ...
        const locationArray = filePath.split('/');
        locationArray.pop();
  ...
  // Image resize
  public async resizeImageBase64(
    directory: string,
    widthString: string = '',
    heightString: string = ''
  ): Promise<any> {
  ...
  public async getFolder(folderName: string = '', vendorPrefix?: string): Promise<any> {
    return new Promise(async (resolve, reject) => {
      ...
      const files = unsanitised.filter((file) =>
        notAllowFolder.length && !vendorPrefix ? !notAllowFolder.includes(file) : true
      );
      ...
      if (folderName !== '') {
          ...
          if (Array.isArray(_file) === false) {
            ...
            if (filesName.includes(folderName.toLowerCase())) {
              const pathfile = path.resolve(
                path.join(process.cwd(), 'uploads', vendorPrefix ? vendorPrefix : '', _file)
              );
              const isDir = await this.isDirCheck(pathfile);
              if (isDir) {
                commonPrefix.push({
            ...
            for (const fileArray of _file) {
              ...
              if (lowerCaseName.includes(folderName.toLowerCase())) {
                commonPrefix.push({
                  Prefix: (vendorPrefix ? `${vendorPrefix}/` : '') + fileArray + '/',
                });
              }
            }
          }
        ...
        for (const file of files) {
          const pathfile = path.resolve(
            path.join(process.cwd(), 'uploads', vendorPrefix ? vendorPrefix : '', file)
          );
          const isDir = await this.isDirCheck(pathfile);
          if (isDir) {
            commonPrefix.push({
